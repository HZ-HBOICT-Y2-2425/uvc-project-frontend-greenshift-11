<script>
  ...
  const fetchAppliances = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3012/appliance/api/appliance-names'); // Correct API endpoint
      ...
      // Combine ids, brands, types, and emojis into a single array of objects
      appliances = responseData.ids.map((id, index) => ({
        id,
        brand: responseData.brands[index],
        type: responseData.types[index],
        emoji: responseData.emojis[index], // Make sure your API includes emojis
      }));
    } catch (err) {
    ...
    {#each appliances as appliance (appliance.id)} <!-- Use `id` as the key -->
      <div class="bg-white rounded-lg shadow-md p-6">
        <div class="mb-4">
          ...
          <p class="text-2xl">{appliance.emoji}</p> <!-- Display the emoji -->
        </div>
